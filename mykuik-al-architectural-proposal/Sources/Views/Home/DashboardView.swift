...
#Preview("DashboardView - Content") {
    NavigationView {
        // Preview uses a static content view since HomeCoordinator is final
...
/// Preview-only content showing dashboard layout
private struct DashboardPreviewContent: View {
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Total Balance Card
                ...
                QuickActionsRow(actions: [
                    QuickAction(title: "Transfer", icon: "arrow.left.arrow.right", action: {}),
                    QuickAction(title: "Cards", icon: "creditcard", action: {}),
                    QuickAction(title: "More", icon: "ellipsis", action: {})
                ])
                .padding(.horizontal)
